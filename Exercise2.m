...
while t < T,
   ...
   rSol    = psolve(rSol,    fluid);
    pressureerror=rSol.pressure-rSolo.pressure;
    er(iter) = norm(pressureerror)/norm(rSol.pressure);
    ers=er(iter);
    iter=iter+1;
   end
   it(end+1)=iter;
   ...
   rSol_pc = psolve(rSol_pc, fluid_pc);
%     report.residual
    pressureerror=rSol_pc.pressure-rSolp.pressure;
    en(iter) = norm(pressureerror);
    ers=er(iter);
    iter=iter+1;
   end
   its(end+1)=iter;
